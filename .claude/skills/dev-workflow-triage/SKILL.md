...
- **Target issue repository**: `seikimatuP/claude-plugins` (hardcoded — change this line to retarget)
...
**Tool path**: use the REST API via `gh api` exclusively for all `seikimatuP/claude-plugins` operations — never the GraphQL-backed `gh issue` porcelain, never GitHub MCP tools (see `§ Step 1 — Pre-flight`'s "every `seikimatuP/claude-plugins` operation … uses the REST API via `gh api`" note for the rationale).
...
- Post the comment via REST: `gh api --method POST /repos/seikimatuP/claude-plugins/issues/<N>/comments -F body=@<path>` (`-F body=@<path>` reads the body verbatim from the staging file)
...
- When closing, use the REST PATCH endpoint: `gh api --method PATCH /repos/seikimatuP/claude-plugins/issues/<N> -f state=closed -f state_reason=completed` (any accepts) or `-f state_reason=not_planned` (all rejects). `state_reason` (note the underscore form `not_planned`) is always available on the REST endpoint, so no gh-version gating is needed — this replaces the GraphQL-backed `gh issue close --reason` porcelain (whose `--reason` flag, and the old `no_reason_flag` gh<2.28 guard, are no longer used)
...
- Run `gh api --method POST /repos/seikimatuP/claude-plugins/issues -f title="<title>" -F body=@<path>` (`gh api` over `gh issue create` for minimum token permissions — `Issues: write` only; same rationale as the dev-workflow producer)
