...
1. **This is the first time the user sees the plan.** Present the reviewed plan to the user (include any unresolved review points from Step 3). Follow the progressive disclosure protocol in [`references/plan-format.md`](references/plan-format.md) § Progressive disclosure at user-gates — render in this order:
   ...
   c. Section inventory per [`references/plan-format.md`](references/plan-format.md) § Progressive disclosure at user-gates. When the user expands a section, render its content following [`references/plan-format.md`](references/plan-format.md) § Localization granularity in the resolved `language` (see §Configuration; default `ja`).
