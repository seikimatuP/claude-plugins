...
1. **This is the first time the user sees the plan.** Present the reviewed plan to the user (include any unresolved review points from Step 3). Lead with one of the literal guidance lines from [`references/plan-format.md`](references/plan-format.md) § Step 4 guidance lines, chosen by (i) whether Decisions has qualifying items and (ii) whether a state file is active. Use the chosen line **verbatim** (no paraphrasing, no concatenation). Write the plan body prose (Overview / Decisions / Design / Test plan / Risks / Unknowns content) in the user's conversation language — match whatever language the user has been using in this session (default `ja`). The English guidance line above and the section heading tokens (`Overview`, `Decisions`, `Design`, `Test plan`, `Risks`, `Unknowns`) are fixed schema labels and stay English verbatim regardless of conversation language. Internal artifacts (commit messages, TodoWrite items, logs) keep the project's default English convention.
