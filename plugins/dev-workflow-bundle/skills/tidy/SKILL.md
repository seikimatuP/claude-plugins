...
`tidy` replicates the cleanup-and-fix behavior the upstream `simplify` skill provided before it was renamed to `code-review` and re-scoped to correctness-only in claude-code v2.1.147. It is a `dev-workflow-bundle` member intended to be invoked from `dev-workflow` Step 6 (or any caller that wants a code-cleanup pass complementary to a correctness review).

The cleanup walk runs in a fresh subagent per iteration (Pattern A — same shape as `verify-diff` and `skill-review`); Edit application stays in the main thread to keep the reviewer bias-free. The skill loops the dispatch + apply cycle until the subagent returns no more `mechanical_edits`, max iterations is reached, or a safety rail trips. Designed to be called from non-interactive routines; it never prompts the user.

**Scope rationale (arbitrary-file vs skill-scoped)**: `dev-workflow` Step 6 is a general post-implementation cleanup pass, not a skill-development pass. Invocation targets are **arbitrary source files** (application code, config, SKILL.md, any text). Unlike `skill-review`, this skill does not restrict the changed-file set to `skills/<name>/` paths.

## Invocation contract

The caller passes these fields in natural language (the skill extracts them from the invocation text):

- `Base ref` *(optional, default `<working-tree-vs-HEAD>`)* — git ref to diff against. When omitted, the skill looks at the working tree's uncommitted + staged + untracked changes (the default scope for `dev-workflow` post-implementation cleanup). When specified (e.g. `Base ref: main`), the skill switches to `git diff <Base ref>` semantics — useful for callers that want to review a stack of already-committed changes between a base branch and HEAD.
...
- `Max iterations` *(optional, default `3`)* — upper bound on the refinement loop. Default `3` mirrors `verify-diff` / `skill-review`; cleanup polish typically converges in 1–2 iterations.

- `Custom instructions` *(optional)* — free-form text injected into the dispatch payload as additional constraints alongside the cleanup checklist. `dev-workflow` Step 6 routes its `custom_instructions` config through this field.
...
5. If `changed_files` is empty, emit the verdict `{"status": "no-actionable-findings", "iterations_used": 0, "applied_edits_count": 0, "notes_remaining_count": 0, "reverted_paths": [], "reason": null}` per `§ Return contract` and stop. `iterations_used: 0` because the iteration loop never runs (mirrors `skill-review` / `verify-diff`'s "Step 1 early returns count as 0" rule).
...
On `i == 1`, use the snapshot from Step 2. On `i ≥ 2`, only re-`Read` the subset of `changed_files` whose path appeared in a successfully-applied `mechanical_edits` entry during iter `i - 1` (untouched files keep their iter-1 snapshot — re-reading them is wasted work and balloons main-thread context — same convention as `skill-review` / `publicity-review` Step 2 (a)). For that same re-read subset, also re-run the per-file `git diff` so the diff payload reflects edits that landed in prior iterations; files outside the subset keep their iter-1 diff.

Invoke the `Agent` tool to dispatch a fresh reviewer. Assemble the dispatch prompt from the five sections below, each framed with a clear `--- LABEL ---` fence (same convention as `verify-diff` § Step 3 (a) and `skill-review` § Step 3 (a)) so the reviewer can parse each payload unambiguously:
...
**Agent unavailable fallback**: detect availability and fall back per the canonical write-up in `rules-review` SKILL.md `§ 5. Review` (the "Detecting Agent availability" / "Fallback when Agent is unavailable" paragraphs). The tidy specialization: when falling back, walk the embedded checklist over each changed file inline-sequentially in the main thread once per iteration and construct the same fenced JSON block defined above so step (b)'s parser handles both paths identically. **Output scope**: the per-iteration JSON produced under fallback is *internal state* for step (b)'s parser — hold it in main-thread context only; do **not** write it into the user-visible response. Only the terminal verdict JSON from `§ Return contract` appears in the response (see the uniqueness clause there).

#### (b) Parse & apply — evaluate in this order, first match wins

Same first-match-wins evaluate-in-order discipline as `verify-diff` § (b) Parse & apply and `skill-review` § (b) Parse & apply. Dispatch failures (Agent tool error / timeout / empty response) are handled in `§ Dispatch failure` before this parse step runs and do not enter sub-cases 1–4 below.
...
This skill follows the same **contract pattern** as `skill-review` § Return contract and `verify-diff` § Step 5 — Emit structured summary: a single fenced JSON block at the very end of the invocation. Only one fenced JSON block must appear **in the user-visible response** — the verdict block — so callers can locate it unambiguously. Intermediate structured outputs that the skill constructs internally for its own parser (e.g., the per-iteration reviewer JSON synthesized under the Agent-unavailable fallback path) are held in main-thread context and do not enter the response stream.
...
- `iterations_used`: number of iterations whose subagent dispatch returned a verdict, **including the iteration whose verdict triggered convergence**. Step 1 early returns (no changed files after exclusion) count as `0`, mirroring `skill-review` / `verify-diff`'s rule
...
When invoked as a sub-skill (i.e. via `Skill(tidy)` from an orchestrator), the fenced JSON verdict block this skill emits is the **structured return value** of the skill's procedure — it is **not** a deliverable to the user, and emitting it does **not** terminate the orchestrator's turn. The same agent that ran this skill must immediately issue the next tool call dictated by the orchestrator's flow (see `dev-workflow` SKILL.md `§ No-Stall Principle` for the canonical caller-side discipline). Do not insert a prose summary, an acknowledgment, or a "shall I proceed?" sentence between the JSON verdict and the next tool call. Only one fenced JSON block — the verdict block — appears in the response, so callers can locate it unambiguously. The skill's own procedure is over; the orchestrator's procedure continues without pause.
...
`references/cleanup-checklist.md` is the source of cleanup rules surfaced to the reviewer. When the upstream `dev-workflow` Step 6 / Step 8 (c) discipline evolves, refresh the checklist file accordingly and ship the refresh as its own commit so the change history is legible.
